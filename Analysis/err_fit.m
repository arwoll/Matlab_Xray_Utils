...
%
% fits x and y to:
%   'y=A*0.5*(1 + erf((x-X0)*2.35/(sqrt(2)*FWHM)))*exp(-(x-X0)/TAU) + BKGD'
%   startpoint is a vector of initial valuesfor [A, X0, FWHM, TAU, BKGD] 


errfun = fittype('A*0.5*(1 + erf((x-X0)*2.35/(sqrt(2)*FWHM)))*exp(-(x-X0)/TAU)+BKGD', ...
    'coeff', {'A', 'X0', 'FWHM', 'TAU', 'BKGD'});


errfit_opts = fitoptions(errfun);
err_sp = startpoint; %[5 -.005 .001 100];     %Estimated values of A, X0, and FWHM
err_lower = [0  -Inf  0 0 0];
err_upper = [Inf  Inf  Inf Inf Inf];
ys_for_wgts = y;
if any(y<=0)
