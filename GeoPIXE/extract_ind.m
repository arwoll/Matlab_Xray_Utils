function det_data = extract_ind(base_path, npts)
...
det_data = zeros(npts, 384, 4096);
seg_dirs = dir(base_path);
seg_dirs = seg_dirs(3:end);
if npts ~= length(seg_dirs)
   fprintf('Nope -- number of seg files does not match expectation -- Aborg\n')
   return
end
abort = 0;
ctr = 0;
for scan_pt = 1:npts
    thisdir = seg_dirs(scan_pt).name;
    seg_files = dir([base_path '/' thisdir]);
    seg_files = seg_files(3:end);
    infname = [base_path '/' thisdir '/' seg_files(1).name];
    fprintf([infname '\n']);
...
%                     is_axis = bitand(bitshift(pe, -29), 7) == 7;
%                     axis = bitand(bitshift(pe, -27), 3);
%                     signbit = int32(bitand(bitshift(pe, -26), 1));
            end
            ...
            while payloadbytes > 0
                ...
                det_data(scan_pt, detN+1, energyADC+1) = ...
                    det_data(scan_pt, detN+1, energyADC+1)+1;
                
%                 fprintf('Found ET event...%d: time: %d, E: %d\n', ...
%                     detN, timeADC, energyADC);
                 ctr = ctr+1;
%                 if (ctr>20)
%                     return
%                 end
