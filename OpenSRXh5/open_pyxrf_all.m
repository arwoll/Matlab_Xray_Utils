...
%
% scandata = open_pyxrf_all(fn)
%    Opens an h5 file generated by SRX's "make_hdf5" script, along with
%    additions made by pyxrf, and creates a matlab structure combining data
%    in openspec-like format, including a "spec" field that contains the
%    normalization counter.
%
%    If there are fit results, open_pyxrf_all also imports these into a
%    substructure, scandata.pyxrf, that has the sorted outputs of those
%    fits (e.g. energy, element & line fit,  fit area, etc).
%
%    If the keyword argument "make_rois" is included,  then the follwoing
%    argument is interpreted as the approximate Energies of each spectral
%    peak. A simple algorithm is followed to bound find the detector
%    channels in which this peak is contained. Then, a gaussian fit is
%    performed to determine the area under this peak. In other words,
%    make_rois echos the function of PyXRF but with zero knowledge of what
%    element it is fitting.
%
%    REQUIRES:
%       * The mcaview version of gauss_fit (to fit, e.g. a 2D array of
%         scans in make_rois
%       * Xray lib & the elam db
*       
%
...
%% Gather info about the lines that were fit
