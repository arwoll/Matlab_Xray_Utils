function [scandata, errors] = openxmap_dev(mcafile, varargin)
...
% For the general case, there can be more than one tiff file per line. The
% file-save policy would most easily ALWAYS have the final index (for file
% number. As well, the looping part below should loop through _groups_ of
% tiff files associated with each line.
%
% The above is implemented. tiff_sort_files above groups files into sets
...
if strcmp(mcaformat, 'g3tiff')
    ...
    mcadata = xmap_read_tiff(mcafiles, 'path', mcapath, ...
        'order', scandata.spec.order, 'reshape', 1);
