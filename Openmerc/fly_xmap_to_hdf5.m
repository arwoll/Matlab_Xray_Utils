function [errors, varargout] = fly_xmap_to_hdf5(fullspecfilename, scann, varargin)
...
mcafiles = dir(fullfile(datapath, ...
    sprintf('%s_*XMAP*_%04d_0000_000.tiff', specscan.specfile, specscan.scann)));
if isempty(mcafiles)
    mcafiles = dir(fullfile(datapath, ...
        sprintf('%s_*XMAP*_%04d_000.tiff', specscan.specfile, specscan.scann)));
    if ~isempty(mcafiles)
       SINGLE_MODE = 1; 
    end
end
...
while ~feof(specfile)
    ...
    if numel(data_cell{1}) ~= chunk_length
       fprintf('Error: seem to have encountered an incomplete line -- not possbile?\n');
       break
    end
    ...
    mcadata = xmap_read_tiff(mcafiles, 'path', datapath, ...
    ...
    if EXPORT_MCADATA && specscan.nslow <= export_lines
        mcaout(specscan.nslow) = mcadata(1);
    end
    flyscan_hdf5_writerow(specscan, specdata, mcadata);
