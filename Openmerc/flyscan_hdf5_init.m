function success = flyscan_hdf5_init(scandata, varargin)
% function flyscan_hdf5_init(scandata, varargin) 
...
%scalarchunk = [length(scalar_labels) scandata.nfast-2]; % For XRF tomo processing only!
...
h5writeatt(h5filename,scangroup,'Header',scandata.scanline)

detector_dataset = '/2D Scan/Detectors';
h5create(h5filename,detector_dataset, [scalarchunk Inf], ...'Inf', ...  scalardims, ...
    'Deflate', deflate_par, 'ChunkSize', [scalarchunk 1] ,'Datatype', 'single') 
...
% scalars = zeros(length(scalar_labels), spec_scalardims(2), spec_scalardims(3), 'single');
...
% 
% h5create(h5filename,'/2D Scan/IC/',size(ic),'Datatype', 'single');
% h5write(h5filename,'/2D Scan/IC/',ic);
% h5writeatt(h5filename,'/2D Scan/IC/', 'DATASET_TYPE','Ion Chamber')
% 
% h5create(h5filename,'/2D Scan/DT_corr_IC/',size(dt_corr_ic),'Datatype', 'single');
