function success = flyscan_hdf5_writerow(scandata, scalar_row, mca_row, varargin)
...
X = scalar_row(1, :)';
Y = scalar_row(2, :)'; % THESE VALUES ARE ASSUMED TO BE IDENTICAL IN EACH ROW
...
if nslow == 1
    ...
    for k=1:length(mca_row)
        ...
        h5create(h5filename,mca_dataset, [mcadims Inf], ...
            'Deflate', deflate_par, 'ChunkSize', [mcadims 1],'Datatype', 'uint16')    
        h5writeatt(h5filename,mca_dataset,'DATASET_TYPE','MCA')
    end
end
% Syntax: h5write(filename,datasetname,data,start,count)
h5write(h5filename,x_dataset, X, [1 nslow], [nfast 1]);
h5write(h5filename,y_dataset, Y, [1 nslow], [nfast 1]);
