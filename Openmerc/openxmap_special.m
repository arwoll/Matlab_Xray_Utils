function [scandata, errors] = openxmap_special(mcafile, varargin)
...
% OK: purpose of below is to handle a (bad) case where the scandata for an
% entire series of measurements was garbage. But there was an identical
% version of the scan that was good. So in that case, we will use the same
% scandata data to give motor positions, etc. 

[specdata, spec_err] = openspec(fullfile(mcapath,specfile), specscan);
...
if isempty(scandata)
    scandata.spec = specdata;
else
   scandata.spec.matfile = matfile;
   scandata.spec.scann = specdata.scann;
   scandata.spec.motor_positions = specdata.motor_positions;
end
