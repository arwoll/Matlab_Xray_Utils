function [q_par q_perp q_par2 q_perp2 z] = open_gid_v3(matfile, E)
% function make_gidplot_v2(matfile, E, varargin) 
%   This version serves to compare true Q calculations with the estimates
%   used in v. 1. 
%   However -- here, unlike v. 2, q_par ONLY refers to the plane
%   perpendicular to the scattering plane, whereas q_perp is essentially Qx
%   + Qz
...
q_par = k*cosb.*sind(nu);
q_perp = k*sqrt((cosa-cosb.*cosnu).^2 + (sina + sind(beta)).^2);
